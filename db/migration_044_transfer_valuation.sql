-- Phase D: taxable branch transfers. Schema only â€” no route reads any of
...
--
-- 1. Account 2106 Inter-Branch Payable â€” the receiving branch's half of the
...
--    different states' books â€” there is no single entity in which they
...
--    states' books on different dates â€” see migration_043's header on why
...
--
-- 2. branches.itc_eligibility â€” whether a branch can claim full input tax
--    credit on what it receives. Three values, though this phase only
--    ever writes FULL:
--
--      FULL           the default. Full ITC unlocks the second proviso to
--                      Rule 28 â€” the invoice value is deemed the open
--                      market value â€” which is why "at cost" is legal and
...
--                      any differently from RESTRICTED today â€” it is a
...
--    the same trade made for stock_transfer_lines.valuation_basis below â€”
--    cheap now, a data migration later if it is not made.
--
-- 3. stock_transfers.to_branch_itc_eligibility â€” the receiving branch's
...
--
-- 4. stock_transfer_lines.valuation_basis â€” which Rule 28 step justifies
...
--    though this phase computes only SECOND_PROVISO â€” see the note on
--    itc_eligibility above for why that is not premature.
--
-- 5. business_partners may now carry bp_type = 'BRANCH' â€” the sub-ledger
...
--    never between two copies of it. Nothing creates these rows yet â€” the
...
--
-- 6. document_number_series â€” a tax invoice under Rule 46 needs a
...
--    encodes the year (GST/IBT/TN/26-27/0001) â€” a row per branch per year
...
--    meant to be set deliberately, once, per branch â€” not invented by
...
--    is a plain row-count today (see routes/salesInvoices.ts) â€” a
--    candidate to move onto this same table later without another one
--    being built.
--
-- Run after migration_043_stock_transfers.sql. Statements stand alone â€”
-- run them one at a time.
--
-- Idempotent: safe to re-run.


-- 1. Rename 1305 to what it now is one half of. Cosmetic â€” no route reads
--    account_name, only account_code â€” but a receivable that still says
...
--    idempotent, unlike ADD CONSTRAINT â€” no drop-then-add dance needed for
--    the column itself.
...
CREATE TABLE IF NOT EXISTS document_number_series (
  ...
  -- "2026-27" style â€” the label that appears in the number, not a date.
