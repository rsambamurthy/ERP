...
// the invoice value of everything dispatched and not yet arrived — cost for
...
router.get("/reconciliation", async (req, res) => {
  ...
  // it is read from the entry the cancellation posted — receiptJournalEntry
  ...
  const inTransit = open.filter((t) => {
    ...
    // CANCELLED — in transit up to the day the return was posted.
...
router.post("/", canPost, async (req, res) => {
  ...
  if (taxable) {
    ...
    // any difference TAXABLE — which is right for "two registrations" and
      ...
      for (const l of parsed) {
        ...
        await tx.stockTransferLine.create({
          data: {
            ...
            ...(taxable ? {
              // Frozen for the HSN summary — see migration_048.
