...
//
// Script 1 of 10: db/migration_054_invoice_charges.sql.
//
// Writes the file only. RUN THE SQL ON RAILWAY, statement by statement,
// before you deploy the code in the scripts that follow - the API will
// reference sales_invoice_charges and accounts 5002/5003/5004 the moment
// it starts.
//
// Save this as backend/tests/chgA.mjs and run it from backend/:
//   node tests/chgA.mjs
...
create("../db/migration_054_invoice_charges.sql", L(
  ...
  "sales_invoice_charges_amount_ck");

console.log(`${applied} change(s) applied, ${already} already there`);
for (const f of ["../db/migration_054_invoice_charges.sql"]) {
