// negD - the test batch. Run negA to negC first.
//
// Creates tests/stkCasesG.mjs - thirteen steps, phase 18, which is BEFORE
// the controls. That placement is the rule rather than a preference: these
// steps move stock and post COGS, so the controls have to run after them.
// negE moves the controls to 19 and the entitlement batch to 20.
//
//   STK-36  Two locks, and neither opens the door alone. Refused with no
//           override; refused WITH the override while the organisation
//           forbids it; refused with the setting on but no reason; posts
//           when both are open; and refused again once it is switched off.
//   STK-37  FIFO refuses even with both locks open, in ORG-B.
//
// The negative balance is deliberately LEFT NEGATIVE. Tidier would be
// worse: STK-30.2 compares the 1201 ledger to the valuation report, and a
// negative item in that comparison is a far stronger claim than none - a
// valuation that silently dropped negative rows would be caught there and
// nowhere else.
//
// Save this as backend/tests/negD.mjs and run it from backend/:
//   node tests/negD.mjs
...
for (const f of ["tests/stkCasesG.mjs"]) {
