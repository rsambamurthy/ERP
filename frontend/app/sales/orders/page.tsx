...
function SalesOrdersInner() {
  ...
  function pickItem(i: number, itemId: string) {
    ...
    setLines((ls) => ls.map((l, idx) => idx !== i ? l : {
      ...
      // the SO itself is in INR. A foreign-currency line starts blank.
  ...
  function updateLine(i: number, patch: Partial<SalesOrderLineInput>) {
    setLines((ls) => ls.map((l, idx) => {
      if (idx !== i) return l;
      const next = { ...l, ...patch };
      // rateFc is authoritative for a foreign-currency SO — rate (INR) is
  ...
  // currency and an SO date selected — same lookup/rationale as the Sales
  // Invoice / Purchase Bill forms.
  useEffect(() => {
    if (!isForeign || !soDate) return;
    let cancelled = false;
    lookupCurrencyRate(currency, soDate)
      .then((res) => {
          ...
          <div className="ent-form-grid" style={{ gridTemplateColumns: isForeign ? "1fr 1fr 2fr" : "1fr 3fr" }}>
            ...
            <div className="ent-fg">
              ...
              <span style={{ fontSize: 12, color: "var(--color-muted)" }}>
                {isForeign
                  ? `Export SO — enter each line's rate in ${currency}. A Sales Invoice raised against this SO will invoice in the same currency.`
                  : "Domestic SO — INR only."}
              </span>
            </div>
          </div>
